...
module.exports = (function levelGenerator() {
  ...
  return {
    ...
    addEntity: function addEntity(type, x, y, target) {
      ...
      this.mapState.set(x, y, newEntity.currentStyle());
    },
    ...
    applyEntities: function applyEntities(entities) {
      ...
      const removeEntities = entities.reduce((graveyard, entity, idx) => {
        ...
        if (timeSinceAnimate > entity.config.speed) {
          ...
          if (entity === this.player() && !entity.attacking) {
            this.actPlayer();
          }
    ...
    actEntity: function actEntity(entity) {
      ...
      if (nextStep) {
        this.mapState.set(...entity.coords, this.map.at(...coords).name);
        // eslint-disable-next-line no-param-reassign
        entity.coords = nextStep;
        const entityZLevel = tiles[entity.currentStyle()].zLevel;
        const zLevelAtTarget = tiles[this.mapState.at(...nextStep).name].zLevel;
        if (entityZLevel > zLevelAtTarget) {
          this.mapState.set(...nextStep, entity.currentStyle());
        }
      }
    },
    ...
    actPlayer: function actPlayer() {
      ...
      const paths = this.mapState.pathsFrom(...player.coords);
    ...
    processProjectile: function processProjectile(projectile) {
      ...
      if (projectile.coords.join(',') === target.coords.join(',')) {
        ...
        // eslint-disable-next-line no-param-reassign
        projectile.die = true;
        this.mapState.set(...target.coords, this.map.at(...target.coords).name);
