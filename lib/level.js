...
module.exports = (function levelGenerator() {
  ...
  const ATTACKS = [
    'firebolt', 'arcanebolt', 'icebolt', 'druidbolt',
  ];

  // types of effects to use for an enemy kill
  const KILL_EFFECTS = [
    'explosion', 'iceshatter', 'arcanestorm', 'heartsplosion', 'natureclaw',
    'poisoncloud',
  ];
  ...
  return {
    ...
    actEntity: function actEntity(entity) {
      // determines next tile to move to, depends on entity target property
      const nextStep = this.determineEntityMove(entity);
      // eslint-disable-next-line no-param-reassign
      if (nextStep) entity.coords = nextStep;
    },
    ...
    determineEntityMove: function determineEntityMove(entity) {
      ...
      if (typeof target === 'string') {
        ...
        toTarget = this.mapState.pathTo(...coords, ...targetEntity.coords, tile => tile.walkable);
      } else if (Array.isArray(target)) {
        toTarget = this.mapState.pathTo(...coords, ...target, tile => tile.walkable);
      }
    ...
    actPlayer: function actPlayer() {
      ...
      // find enemies in range that have a connected path to them
