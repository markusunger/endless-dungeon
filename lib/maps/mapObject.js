...
module.exports = (function mapClass() {
  return {
    ...
    overlayableAt: function overlayableAt(x, y) {
      // checks if tile should be overlayed by an effect
      return !(this.outOfBounds(x, y) || !this.at(x, y).walkable);
    },
    ...
    pathsFrom: function pathsFrom(x, y) {
      const pf = Object.create(pathfinder);
      return pf.init(this).run(x, y);
    },
  };
}());
