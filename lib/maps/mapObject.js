/*
  The base prototype object for a map, includes pathfinding convenience methods
*/
...
module.exports = (function mapClass() {
  return {
    ...
    randomWalkableTile: function randomWalkableTile() {
      // finds one random walkable tile
      let tile;
      while (!tile) {
    ...
    randomWallTile: function randomWallTile() {
      let tile;
      while (!tile) {
        const x = Math.floor(Math.random() * this.height);
        const y = Math.floor(Math.random() * this.width);
        if (this.at(x, y).name.startsWith('wall')) tile = [x, y];
      }
      return tile;
    },
    ...
    borderTile: function borderTile(x, y) {
      return (
        x === 0 || y === 0
    ...
    pathTo: function pathto(x, y, targetX, targetY, cb) {
      const pf = Object.create(pathfinder);
      return pf.init(this).runAStar(x, y, targetX, targetY, cb);
    },
  };
}());
