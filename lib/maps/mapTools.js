...
module.exports = (function mapTools() {
  // allowed prefabs
  const PRE_LIST = [
    'pool', 'lavalake', 'magicchamber', 'goldchamber', 'deadend', 'library',
    'pillars', 'dining', 'fountains', 'pond', 'well',
  ];
  ...
  return {
    ...
    mapRotate: function mapRotate(map) {
      const rows = map.length;
      const cols = map[0].length;

      let newMap = new Array(cols).fill(0);
      newMap = newMap.map(() => new Array(rows).fill(0));
    ...
    addPrefab: function addPrefab(map) {
      ...
      while (true) {
        // filter appropriate prefabs from list of all
        const prefabName = prefabList[Math.floor(Math.random() * prefabList.length)];
        const prefab = prefabs[prefabName];

        // rotate prefab randomly for more variety
        const rotation = [0, 1, 2, 3][Math.floor(Math.random() * 4)];
        for (let i = 0; i <= rotation; i += 1) {
          prefab.map = this.mapRotate(prefab.map);
        }
        ...
        if (origin) {
          const newEntities = prefab.map.reduce((entities, row, y) => {
            row.forEach((tile, x) => {
              let toPlace = prefab.lookup[tile];
              if (Array.isArray(toPlace)) {
                toPlace = toPlace[Math.floor(Math.random() * toPlace.length)];
              }
          ...
          const connectors = [
            origin,
            [origin[0], origin[1] + prefab.map[0].length - 1],
            [origin[0] + prefab.map.length - 1, origin[1]],
            [origin[0] + prefab.map.length - 1, origin[1] + prefab.map[0].length - 1],
          ];
          const connector = connectors[Math.floor(Math.random() * connectors.length)];
          map.set(...connector, 'floor');
          this.connect(map, ...connector);
