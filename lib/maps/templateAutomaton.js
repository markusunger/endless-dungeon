...
module.exports = (function cellularAutomaton() {
  ...
  return function generate(width, height) {
    ...
    let entry = template.findEntry();
    while (template.countNeighbors(...entry, tile => tile.type === 'floor') < 1) {
      entry = template.findEntry();
    }

    let exit = template.findExit();
    while (template.countNeighbors(...exit, tile => tile.type === 'floor') < 1) {
      exit = template.findExit();
    }

    template.set(...entry, { type: 'floor' });
    template.set(...exit, { type: 'floor' });
