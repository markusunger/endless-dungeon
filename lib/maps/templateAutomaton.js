  ...
  return function generate(width, height) {
    ...
    for (let i = 0; i < template.height; i += 1) {
      for (let j = 0; j < template.width; j += 1) {
        const newType = Math.random() > WALL_CHANCE ? 'floor' : 'wall1';
