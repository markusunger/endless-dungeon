...
module.exports = (function cellularAutomaton() {
  const WALL_CHANCE = 0.4; // the chance of a wall placement in the initial setup
  const ITERATIONS = 4; // the number of times the CA algorithm runs
  ...
  return function generate(width, height) {
    ...
    for (let i = 0; i < ITERATIONS; i += 1) {
      const newTemplate = Object.create(map); // generate empty map to create new state to
      ...
      for (let j = 0; j < height; j += 1) {
        for (let k = 0; k < width; k += 1) {
          ...
          if (neighbors >= WALL_EVOLUTION) {
            ...
            newTemplate.set(j, k, newTemplate.at(j, k));
          } else {
            newTemplate.set(j, k, { type: 'floor' });
          }
        }
