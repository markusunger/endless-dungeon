...
module.exports = (function cellularAutomaton() {
  ...
  const ITERATIONS = 5; // the number of times the CA algorithm runs
  ...
  function createMap(width, height, empty = true) {
    /*  creates a two-dimensional array with the dimension height x width
          if empty is true:
            all array elements are {type: floor}
          else:
            random distribution of {type: floor/wall} according to WALL_CHANCE */
  ...
  function countNeighbors(x, y, map) {
    /* looking at all 8 neighbor positions (up/down, left/right), diagonal) and
