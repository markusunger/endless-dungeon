...
module.exports = (function DrunkenCarver() {
  ...
  return function generate(width, height, entry) {
    ...
    template.create().fill({ type: 'wall1' }); // create fully walled map

    // if no entry point was provided (first level), create one randomly
    const entryPoint = entry || template.findEntry();

    // create exit point with enough distance to entry to make an interesting map
    let exitPoint = template.findExit();
    while (determineDifference(entryPoint, exitPoint) < template.height) {
      exitPoint = template.findExit();
    }

    // carve out entry and exit
    template.set(...entryPoint, { type: 'floor' });
    template.set(...exitPoint, { type: 'floor' });


    let carver = entryPoint;

    while (determineDifference(carver, exitPoint) !== 0) {
      ...
      const currentDiff = determineDifference(carver, exitPoint);
      ...
      template.set(...carver, { type: 'floor' });
    }
