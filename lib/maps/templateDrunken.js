...
module.exports = (function DrunkenCarver() {
  ...
  return function generate(width, height, entry) {
    ...
    const exitPoint = mapHelpers.setExit(width, height);
    ...
    while (carver[0] !== exitPoint[0] || carver[1] !== exitPoint[1]) {
      ...
      const currentDiff = determineDifference(carver, exitPoint).reduce((acc, p) => acc + p);
      ...
      const directions = ADJACENT.filter((relativePosition) => {
        ...
        const newDiff = determineDifference([newX, newY], exitPoint).reduce((acc, p) => acc + p);
