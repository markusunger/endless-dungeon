...
module.exports = (function DrunkenCarver() {
  ...
  function createMap(width, height) {
    ...
    return map.map(() => new Array(width).fill(0).map(() => ({ type: 'wall' })));
  }

  function setEntry(width, height) {
    // determines a random entry position for the carver if none was provided
    let entryX;
    let entryY;
    if (Math.random() >= 0.5) { // entry on bottom
      entryX = height - 1;
      entryY = Math.ceil(Math.random() * (width - 2)); // prevent outermost tiles from becoming entry
  ...
  return function generate(width, height, entry) {
    ...
    const entryPoint = (entry === undefined) ? setEntry(width, height) : entry;
    const exit = setExit(width, height);
