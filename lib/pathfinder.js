...
module.exports = (function pathfinder() {
  return {
    ...
    runBFS: function runBFS(rootX, rootY, cb) {
      ...
      while (explore.length > 0) {
        ...
        const neighbors = this.map.neighbors(...current, tile => cb(tile));
    ...
    runAStar: function runAStar(rootX, rootY, targetX, targetY, cb) {
      ...
      while (explore.length > 0) {
        ...
        const neighbors = this.map.neighbors(...current.coords, tile => cb(tile));
